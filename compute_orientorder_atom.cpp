...
/* ----------------------------------------------------------------------
   Contributing author:  Spencer Ortega
------------------------------------------------------------------------- */
...
{
  //cout << endl << endl << ">>>>>>>> t=" << t << " / mpi rank=" << comm->me << " <<<<<<<<<<<" <<endl;
    ...
    if (mask[i] & groupbit) {
      ...
      for (jj = 0; jj < jnum; jj++) {
        ...
        if (rsq < cutsq) {
          ...
          // j = a neighbor of atom i
          //myfile << "  jlocal=" << j << " , jtrue=" << atom->tag[j] << endl; 

          // int jtemp = j;
          // myfile << "    ";
          // while(atom->sametag[jtemp] >= 0){
          //   myfile << jtemp << "/" << atom->tag[jtemp] <<" --> ";
          //   jtemp = atom->sametag[jtemp];
          // }
          // myfile << endl;
          ...
          rlist[ncount][2] = delz;
          //nearest[ncount++] = j;
          nearest[ncount++] = atom->tag[j];
        }
      }

      // myfile << endl << "Duplicates: ";
      // for(int iii = 0; iii < ncount-1; iii++){
      //   for(int jjj = iii+1; jjj < ncount; jjj++){
      //     if(nearest[iii] == nearest[jjj]){
      //       myfile << nearest[iii] << " , ";
      //     }
      //   }
      // }

      myfile << endl << "is atom i in neighbor list?: ";
      for(int iii = 0, x = atom->tag[i]; iii < ncount; iii++){
        if(nearest[iii] == x){
          myfile << "YES!" <<endl;
          break;
        }  
      ...
      if(t == 0){
        //save neighbor list
        // t0[ii] = new int[ncount]
        //cout << "writing neighbor list at t0 for atom " << ii << endl;
        // arr[ii] = new int[ncount];
        // counts[ii] = ncount;
        // for(int index = 0; index < ncount; index++){
        //   arr[ii][index] = nearest[index];
        // }
        //cout << "finished writing neighbor list at t0 for atom " << ii << endl;
        //error->all(FLERR,"End of t=0");

      }
      else{
        // cout << "retrieving neighbor list at t0 for atom " << ii << endl;
        // for(int index = 0; index < counts[ii]; index++){
        //   //cout << arr[ii][index] << ", ";
        // }
      }
