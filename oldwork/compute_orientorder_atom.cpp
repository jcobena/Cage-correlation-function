  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    if (mask[i] & groupbit) {
      ...
      if(t == 0){
        ...
        //allocate memory for atom i neighbor list
        nls[ii] = new int[ncount];

        // copy data
        for(int index = 0; index < ncount; index++){
           nls[ii][index] = nearest[index];
        }
      }
      else{
        ofstream myfile;
        ...
        name = "t-" + time + "_" + "mpi-" + name + ".txt";
        myfile.open(name.c_str());
        myfile << "nmax=" << atom->nmax<<endl;

        //int sum=0;
        for(int iii=0; iii < count; iii++){
          myfile << "atom " << owned[iii] << endl;
          myfile << "size " << nlsize[iii] << endl;
          //sum += nlsize[iii]; 
          for(int jjj=0; jjj < nlsize[iii]; jjj++){
            myfile << nls[iii][jjj] << ", ";
          }
          myfile << "\n\n";
        }
        //myfile << "total sum: " << sum << endl;
        myfile.close();
